...
//--------------------------------------------------------------------------------------
LinkedList* ll_filter(LinkedList *arrayLibros, int (*pFunc)(void*))
{
	...
	if (arrayLibros != NULL && *pFunc != NULL)
	{

		newLinkedList = ll_newLinkedList();

		if (newLinkedList != NULL) {
			tam = ll_len(arrayLibros); //guardo el tamanho de la lista original

			for (int i = 0; i < tam; i++)
				{
				pElement = ll_get(arrayLibros, i); //recorro la lista original.
				if (pFunc(pElement))
				{ // si el callback devuelve 1, me copio el elemento.
